...
export function Scene() {
  ...
  useLayoutEffect(() => { if (dof.current) dof.current.target = focus })
  ...
  return (
    <Canvas
      ...
      shadows
      dpr={[1, 1.75]}
      camera={{ fov: 35, near: 0.1, far: 60, position: [-0.6, 1.75, 4.2] }}
      gl={{ antialias: false, powerPreference: 'high-performance' }}
      onCreated={() => useStore.getState().set({ ready: true })}
    >
      ...
      <EffectComposer multisampling={0}>
        <DepthOfField ref={dof} focalLength={0.03} bokehScale={4} height={600} />
        <Bloom mipmapBlur luminanceThreshold={0.85} intensity={0.9} />
